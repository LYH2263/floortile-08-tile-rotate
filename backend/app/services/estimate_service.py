...
def run_estimate(
    ...
    # Rotation not declared on the request falls back to the system default;
    # the resolved flag is what gets returned and persisted with the run.
    rotated = settings_repo.get_default_rotate() if rotate is None else bool(rotate)
    calc = tile_count(
        room["length"], room["width"], tile["tile_l"], tile["tile_w"], waste, rotate=rotated
    )

    run_id = None
    if save:
        payload = {**calc, "room_id": room_id, "tile_id": tile_id}
        run_id = history.insert_run(room_id, tile_id, waste, rotated, payload, note)
