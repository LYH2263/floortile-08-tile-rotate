...
onMounted(async () => {
  ...
  rotated.value = ['1', 'true', 'yes', 'on'].includes(String(settings.default_rotate).toLowerCase())
})

async function preview() {
  err.value = ''
  try {
    result.value = await getJSON(`/api/estimate?room_id=${roomId.value}&tile_id=${tileId.value}&rotate=${rotated.value}`)
  } catch (e) {
    err.value = e.message
    result.value = null
  }
...
<template>
  <div class="page">
    ...
    <label>旋向 <input type="checkbox" v-model="rotated" @change="preview"> 旋转90°</label>
